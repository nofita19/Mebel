...
class Barang extends CI_Controller {
    ...
    public function tambah(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $cek = $this->v->GetWhere('barang', ['barang_kode   ' => $this->input->post('barang_kode')]);
            ...
            if (!$cek) {
                $this->v->insert("barang" ,$data);
				$_SESSION['msg'] = ['info', 'Data Berhasil Di Tambahkan !!'];
				redirect("Barang");
			}else{
				$_SESSION['msg'] = ['danger', 'Data Sudah Tersedia !!!'];
    ...
    public function getdataotomatis()
    {  
        $id = $_GET['kode_barang'];
        $get = $this->db->query("SELECT * FROM barang WHERE barang_kode='$id'")->result_array()[0];
        
        if ($get > 0) {
            $data = array(
                'barang_nama' => $get['barang_nama'],
                'jenis_bahan' => $get['jenis_bahan'],
                'type_barang' => $get['type_barang'],
                'biaya_produksi' => $get['biaya_produksi'],
                'biaya_tukang' => $get['biaya_tukang'],
                'biaya_distribusi' => $get['biaya_distribusi'],
                'biaya_lainlain' => $get['biaya_lainlain'],
                'keuntungan' => $get['keuntungan'],
                'stok' => $get['stok'],
                'harga_asli' => $get['harga_asli'],
            );
        }else{
            $data = array(
                'barang_nama' => "",
